...
class conv(nn.Module):
    def __init__(
        self, num_in_layers, num_out_layers, kernel_size, stride, activate=None
    ):
...
class DAA(nn.Module):
    ...
    def _smooth_l1_loss(self, input, target, weight=1.0, reduce=True):
    ...
    def run_loss(self, ages, labels, accuracy_threshold):
        ...
        loss["accuracy"] = (
            100 * (age_diff <= accuracy_threshold).float().sum() / labels.size(0)
        )
    ...
    def forward(self, x, run_info):
        ...
        if run_info["mode"].lower() != "test":
            loss = self.run_loss(
                ages.view(-1),
                run_info["labels"].view(-1),
                run_info["accuracy_threshold"],
            )
            results["loss"] = loss
        else:
